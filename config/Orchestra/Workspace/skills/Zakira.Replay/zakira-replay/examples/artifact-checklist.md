...
- Known codes (one line per code, severity in parentheses):
  ...
  - `CAPTURE_BROWSER_MEDIA_DOWNLOADED` (info) / `CAPTURE_BROWSER_MEDIA_NO_CANDIDATE` (info) / `CAPTURE_BROWSER_MEDIA_DOWNLOAD_FAILED` (warning) — STT-fallback media download via the authenticated browser context. Triggered only when `stt: true` is requested AND no captions came out of the other layers AND no audio source exists. DASH / HLS chunked streams (typical SharePoint Stream pattern) emit `CAPTURE_BROWSER_MEDIA_NO_CANDIDATE` and skip STT cleanly.
